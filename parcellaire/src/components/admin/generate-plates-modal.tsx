...
import {
  Loader2, X, Printer, Check, AlertTriangle,
} from "lucide-react";
...
export function GeneratePlatesModal({ parcelles, onClose }: GeneratePlatesModalProps) {
  ...
  // Filter out parcelles that already have a variant assigned
  ...
  const variants: VariantDesign[] = selectedTemplate
  ...
  async function handleGenerate() {
    if (!selectedTemplateId || selectedVariantIdx === null) {
      toast.error("Sélectionnez un template et une variante");
      return;
    }
    if (eligible.length === 0) {
      toast.error("Aucune parcelle éligible");
      return;
    }

    setGenerating(true);
    try {
      const res = await fetch("/api/parcelles/generate-plates", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          parcelleIds: eligible.map((p) => p.id),
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">
                {eligible.length} parcelle(s) éligible(s)
              </p>
            </div>
          </div>
          ...
          {alreadyAssigned.length > 0 && (
            <div className="flex items-start gap-2 p-3 rounded-lg bg-amber-50 border border-amber-100">
              ...
              <p className="text-xs text-amber-800">
                <strong>{alreadyAssigned.length}</strong> parcelle(s) ont déjà une variante assignée et seront ignorées (non modifiable).
              </p>
            </div>
          )}
          ...
          {result && (
            <div className="p-4 rounded-xl bg-emerald-50 border border-emerald-100 text-center">
              ...
              <p className="text-sm font-medium text-emerald-800">
                {result.generated} plaque(s) générée(s) avec succès
              </p>
              {result.failed > 0 && (
                <p className="text-xs text-red-600 mt-1">{result.failed} échec(s)</p>
              )}
          ...
          {!result && (
            <>
              ...
              <div>
                ...
                {loadingTemplates ? (
                  ...
                  <p className="text-xs text-gray-400 py-4 text-center">
                    Aucun template. Créez-en un dans Plaques Templates.
                  </p>
                ) : (
                  <div className="space-y-2 max-h-[150px] overflow-y-auto">
                    {templates.map((t) => (
                      <Card
                        ...
                        onClick={() => { setSelectedTemplateId(t.id); setSelectedVariantIdx(null); }}
                      >
                        <CardContent className="p-3 flex items-center gap-3">
                          {t.sealUrl && (
                            <div className="w-8 h-8 rounded-full border border-gray-200 overflow-hidden flex-shrink-0">
                              {/* eslint-disable-next-line @next/next/no-img-element */}
                              <img src={t.sealUrl} alt="" className="w-full h-full object-cover" />
                            </div>
                          )}
                          <div className="flex-1 min-w-0">
                            <p className="text-sm font-medium text-gray-900 truncate">{t.nom}</p>
                            <p className="text-[10px] text-gray-400">
                              {(typeof t.variants === "string" ? JSON.parse(t.variants) : t.variants).length} variante(s)
                            </p>
                          </div>
                          {selectedTemplateId === t.id && (
              ...
              {selectedTemplate && variants.length > 0 && (
                <div>
                  <p className="text-xs font-semibold text-gray-700 mb-2">
                    2. Choisir une variante (non modifiable après assignation)
                  </p>
              ...
              {selectedVariantIdx !== null && (
                <div className="p-3 rounded-lg bg-gray-50 border border-gray-200">
                  <div className="flex items-center justify-between text-xs">
                    <span className="text-gray-600">Parcelles à générer :</span>
                  ...
                  <div className="flex items-center justify-between text-xs mt-1.5">
                    ...
                    <span className="font-medium text-gray-900">{selectedTemplate?.nom}</span>
                  </div>
                  <div className="flex items-center justify-between text-xs mt-1.5">
                    ...
                    <div className="flex items-center gap-1.5">
                      <div
                        ...
                        style={{ backgroundColor: variants[selectedVariantIdx]?.bgColor }}
                      />
                      <span className="font-medium text-gray-900">
                        {variants[selectedVariantIdx]?.name}
                      </span>
                    </div>
                  </div>
                </div>
              ...
              <div className="flex items-center gap-3 pt-2">
                <Button variant="outline" onClick={onClose} disabled={generating} className="flex-1 h-10">
                  Annuler
                </Button>
                <Button
                  onClick={handleGenerate}
                  disabled={generating || !selectedTemplateId || selectedVariantIdx === null || eligible.length === 0}
                  className="flex-1 h-10 bg-gradient-to-r from-emerald-500 to-green-600 hover:from-emerald-600 hover:to-green-700 text-white shadow-md shadow-emerald-500/20"
                >
                  {generating ? (
